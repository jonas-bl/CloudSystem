...
public class SpigotServerManager {
    ...
    public CloudServer createNewServerFromTemplate(ServerTemplate serverTemplate, ServerTemplate copyTemplate, UUID serverUUID) {
        int port = getAvailablePort();
        int id = getAvailableServerId(serverTemplate.getName());
        CloudServerManager.getInstance().getUsedSpigotPorts().add(port);
        CloudServer cloudServer = new CloudServer(serverTemplate, copyTemplate, id, CloudSlaveManager.getInstance().getSlaveByName(serverTemplate.getSlave()).getAdress(), port, serverTemplate.getRam(), serverUUID, ServerState.OFFLINE, null, null);
    ...
    */
    ...
    public synchronized void cloneTemplate(CloudServer server) {
        try {
            if (!server.isStatic()) {
                ...
                if(server.getCopyTemplate() != null) {
                    FileManager.copyFolder(FileManager.getTemplatesDirectory(server.getCopyTemplate().getName()).toPath(), FileManager.getTempDirectory(server.getTempName()).toPath());
                }
                FileManager.copyFolder(FileManager.getTemplatesDirectory("Global/Spigot").toPath(), FileManager.getTempDirectory(server.getTempName()).toPath());
            }
    ...
    public void startServer(CloudServer server) {
        File file = server.isStatic() ? new File("static/" + server.getName() + "/spigot.jar") : new File("temp/" + server.getTempName() + "/spigot.jar");
        if (!file.exists()) {
            CloudLogger.getInstance().log("[Error] Could NOT start server " + server.getName() + " because spigot.jar does not exist in temp directory.");
            return;
        }
